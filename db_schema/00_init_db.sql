...
FOR EACH ROW
BEGIN
    UPDATE domain_ips SET last_updated = CURRENT_TIMESTAMP
    WHERE domain_id = old.domain_id AND ip_id = old.ip_id;
END;
...
CREATE INDEX IF NOT EXISTS idx_subdomain_ips ON subdomain_ips (
    subdomain_id, ip_id
);
...
FOR EACH ROW
BEGIN
    UPDATE subdomain_ips SET last_updated = CURRENT_TIMESTAMP
    WHERE subdomain_id = old.subdomain_id AND ip_id = old.ip_id;
END;
...
CREATE TABLE IF NOT EXISTS endpoints (
    ...
    active BOOLEAN GENERATED ALWAYS AS (
        status_code IS NOT NULL AND status_code != 404
    ) STORED,
    content_length INT,
    content_type TEXT,
...
CREATE INDEX IF NOT EXISTS idx_endpoints_url_code ON endpoints (
    subdomain_id, url, status_code
);
